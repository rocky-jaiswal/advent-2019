...
private fun calc(input: List<Int>) {
    ...
    while(true) {
        ...
        if (code > 10) {
            // special code
            val opcode = code % 10
            val paramModes = when (val modes = code / 100) {
                in 0..9 -> listOf(0, 0, 1)
                in 10..99 -> listOf(0, 1, modes % 10)
                else -> {
                    println("code - $code")
                    listOf(1, modes % 100, modes % 10)
                }
            }

            if(opcode == 1) {
                mutableInput[i + 3] = if(paramModes[2] == 0) mutableInput[mutableInput[i+1]] else mutableInput[i+1] + if(paramModes[1] == 0) mutableInput[mutableInput[i+2]] else mutableInput[i+2]
                i += 4
            }
            if(opcode == 2) {
                mutableInput[i + 3] = if(paramModes[2] == 0) mutableInput[mutableInput[i+1]] else mutableInput[i+1] * if(paramModes[1] == 0) mutableInput[mutableInput[i+2]] else mutableInput[i+2]
            ...
            if(opcode == 4) {
                println(if(paramModes[2] == 1) mutableInput[i + 1] else mutableInput[mutableInput[i + 1]])
                i += 2
            }
        }
    }
